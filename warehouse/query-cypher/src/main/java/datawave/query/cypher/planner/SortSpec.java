...
/**
 * One ORDER BY item in the logical plan: the projected alias to sort on and
 * the sort direction. ORDER BY is applied in-memory after the full hop chain
 * completes; only projected aliases (not arbitrary expressions) are accepted
 * in M2.
 */
